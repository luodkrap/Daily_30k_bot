...
| 항목 | 값 |
|------|----|
| **현재 Phase** | **⏸️ 일시 중단 (2026-06-04 KST, 도울님 결정 — 검증 정체·수익 불확실로 피로).** 봇 stop+disable, Lightsail Stop 안내. paper 9일 무거래는 버그 아님(200MA 약세장 진입 차단=의도된 동작). 재개 절차·진단은 아래 "현재 작업" 참조. 이하는 중단 직전 Phase 7 맥락 보존 → **Phase 7 — mainnet paper 모드 구현 완료·배포 (2026-05-25 KST)**. testnet 결함 3종(가격 -9% 괴리·얇은 호가 미체결·일봉 20개<201)이 모두 외부 환경이라 코드로 불가 → **mainnet 실시세를 읽되 주문만 로컬 가상 체결하는 `MODE=paper`** 신규. 신규 `paper_exchange.py`(`PaperExchange` — ccxt.binance 덕 타이핑: 읽기는 mainnet reader 위임, 주문/잔고는 로컬 가상 체결, JSON 영속화). **executor·screener·persistence 무수정**(거래소 객체만 교체). config(MODE assert+paper 키 분기+PAPER_STATE_PATH/SLIPPAGE), main(exchange 주입 분기), executor(`recover_state` paper 가드 — 가상 상태는 JSON 복원이라 clean-slate 청산 스킵). 체결 MVP(지정가 터치=전량 체결, 시장가=현재가±0.05% 슬리피지, FEE_RATE 양방향). 가상 상태(잔고·미체결)는 `paper_state.json` 영속화 → 재시작 복원. **스모크 테스트(실 mainnet $77,558): 현재가 지정가 즉시 체결 확인 → testnet 미체결 idle 문제 근본 해소.** 회귀 테스트 5건 + 전체 통과. 다음: `.env MODE=paper` + `BINANCE_API_KEY` 없이도 가동(public 데이터). **B3 = live 전환 NO 유지**, paper 누적으로 전략 재검증 후 재판단. **배포**: `git push` → `bash deploy/update.sh` → `.env MODE=paper` → `sudo systemctl restart daily30k`. 이전 N30/N29 이력은 파일 변경 이력 참조. (구) Phase: N30 진입 실패 idle·200MA 필터 미적용 가시화 패치 완료. 도울님 "페이퍼 잘 되나" 질문 → Supabase 직접 진단으로 **05-15 22:52 이후 10일간 무거래(silent idle)** 발견. py-spy dump(PID 61100, uptime 10일 17시간) 결과 **hang 아님** — event loop(epoll idle)·screener(15분마다 타겟 선정)·heartbeat·snapshot(30분마다) 전부 정상. 근본 원인: executor 루프 ── 8단계 `setup_grid` **초기 지정가 매수가 testnet 얇은 호가에서 미체결 → `return`(is_active=False) → engine=None → 60초 후 무한 재시도**. trades 05-15 이후 BUY 0건·잔고 BTC 없음·열린 주문 0건으로 확정. 이 "거래 진입 실패 반복" idle 은 N25 trade-idle(engine 활성 전제, [executor.py:120](executor.py#L120) `engine is None` 에서 return)·snapshot watchdog(snapshot 정상) 어느 것도 못 잡는 사각지대였음 → 10일간 N25_TRADE_IDLE 0건. 부수 발견: testnet BTC/USDT **일봉 20개<201** 로 `update_market_filter` 가 매번 즉시 return → `is_market_healthy` True 동결·MARKET_FILTER 이벤트 0건(관측 일치). 텔레그램 NetworkError 56건은 PTB `network_retry_loop` 자동 재시도(정상)라 변경 불필요. **N30 패치(가시성, 거래 로직 불변)**: ① `_note_grid_setup_idle()` 신규 — ── 8 진입 실패가 1시간 이상 지속 시 GRID_SETUP_IDLE WARNING(throttle 6h, stdout/텔레그램/Supabase), 진입 성공 시 `executor_grid_idle_since=0` 리셋. ② `update_market_filter` 일봉<201 시 MARKET_FILTER_UNAVAILABLE WARNING 1회(동작=매매 허용 유지). 회귀 테스트 2건(`test_n30_grid_setup_idle_alerts_after_threshold`, `test_n30_market_filter_unavailable_warns_once`) + 전체 통과. **누적 testnet 실현 PnL -233,846원(순손실) → 오늘 B3 = live 전환 NO 명확.** testnet 호가·가격 피드·일봉 결함은 외부 환경이라 코드로 불가 → **mainnet paper 전환 검토가 다음 핵심 의사결정**. **배포 절차**: `git push` → SSH `bash deploy/update.sh` → `sudo systemctl restart daily30k`(.service 미변경이라 daemon-reload 불필요). 그 전 5/13~5/15 N29 이력은 파일 변경 이력 참조. |
| **마지막 점검** | 2026-06-04 KST (⏸️ 프로젝트 일시 중단 / paper 9일 무거래 = 200MA 약세장 차단 확인, 버그 아님 / 봇·인스턴스 정지) |
| **점검 누적** | 7/7 |
...
> **⏸️ 프로젝트 일시 중단 (2026-06-04 KST, 도울님 결정).** 사유: 장기 검증 정체·수익 가능성 불확실로 인한 피로. 나중에 재개 가능성 있음.
>
> **중단 시 상태 (재개 시 참조):**
> - 봇 `sudo systemctl stop daily30k && disable` 완료 → `inactive`/`disabled` (인스턴스 Start 해도 자동으로 안 켜짐).
> - ✅ **2026-06-04 Lightsail 인스턴스 삭제 완료 → 과금 0 확정.** 고정 IP 는 없었음(3.36.26.177 은 dynamic IP 였고 인스턴스와 함께 소멸 → 추가 release 불필요). ⚠️ 교훈: Lightsail 은 EC2 와 달리 **stop 해도 월정액($7) 계속 청구** → 삭제만이 과금 0. 코드(GitHub/로컬)·데이터(Supabase Free) 보존되므로 손실 없음.
> - **paper 9일(05-25~06-04) 최종 진단**: 봇·screener·snapshot·heartbeat 전부 정상이나 **거래 0건, equity 2,222.22 USDT 동결**. 원인 = **버그 아님**. mainnet 일봉 201개로 200MA 필터가 제대로 작동 → BTC가 200일선 -16.9% 하회한 약세장 9일 내내 신규 진입을 차단(의도된 안전장치). testnet(일봉 20개로 200MA 무력화)과 정반대로 paper는 올바르게 동작했으나, 마침 약세장이라 전략 검증 데이터가 0건. B3 판단 무기한 보류.
>
> **재개 절차 (인스턴스 삭제했으므로 재구축 필요)**: Lightsail 에서 **새 인스턴스 생성**(Ubuntu, 서울 ap-northeast-2, 1GB+) → 새 고정 IP attach → SSH 후 `git clone` → `deploy/setup.sh` 실행 → `.env` 재작성(`MODE=paper`, 텔레그램 토큰/chat_id, `SUPABASE_DB_URL` — 기존 Supabase 그대로 재사용) → `sudo systemctl enable --now daily30k` → `journalctl -u daily30k -f` 로 `[MODE=PAPER]` 확인. SSH 키·IP 는 새로 발급되므로 [[reference-server-access]] 메모리도 갱신할 것. BTC 가 200일선 위 강세장이어야 paper 거래 시작(약세장이면 무거래 = 정상).
>
> **(구) 다음 세션 첫 액션 — 옛 paper 배포 절차, 재개 시 참고용**:
> 1. **paper 모드 배포** — `git push` → SSH `cd ~/Daily_30k_bot && bash deploy/update.sh` → `.env` 에 **`MODE=paper`** 설정(testnet 키 불필요, mainnet public 데이터 사용) → `sudo systemctl restart daily30k` → `sudo systemctl status daily30k --no-pager`. (선택: `PAPER_STATE_PATH`/`PAPER_SLIPPAGE` 기본값 사용)
> 2. 부팅 후 `journalctl -u daily30k -f` 로 확인: `[paper] 초기 시드 USDT=...` 또는 `[paper] state 복원 ...` / `[init_db] backend=supabase` / `[MODE=PAPER]` 텔레그램. screener 가 **mainnet 실타겟**(예: BTC/ETH/SOL, MIN_VOLUME 100M) 선정 → `setup_grid` 가상 그리드 배치 → `[그리드] ... 배치 완료` (testnet 과 달리 초기 매수 즉시 체결).
> 3. **관찰 (며칠~2주)** — Supabase `trades`/`equity_snapshots`/`bot_events` 에 **`mode='paper'`** 로 누적 확인. 실시장에서 그리드 전략이 +/− 인지 검증. `paper_state.json` 이 재시작 후에도 잔고·미체결 복원하는지 확인.
> 4. **B3 재판단** — paper 누적 손익이 의미있게 +/안정적이면 그때 비로소 live 전환(소액부터) 검토. testnet 누적(-233,846원)은 외부 결함 오염이라 폐기, paper 부터 새 카운터.
> 5. 잔여 가시성 감시 — paper 에서도 GRID_SETUP_IDLE / N25_TRADE_IDLE / SUPERVISOR_RESTART 동일 작동(코드 공유).
...
| 날짜 | 파일 | 변경 이유 |
|------|------|-----------|
| 2026-05-26 | [deploy/schema.sql](deploy/schema.sql) | **paper 핫픽스 — DB CHECK 제약** — paper 배포 직후 텔레그램 `CheckViolationError: {bot_events,equity_snapshots,trades}_mode_check` 연발. 원인: 3개 테이블 `mode ... CHECK (mode IN ('live','testnet'))` 가 `'paper'` 거부 → paper 기록 전량 차단(봇은 persistence N2 실패 격리로 생존·매매 무영향, 단 Supabase 적재 0건). paper 탐색 시 persistence.py 코드만 보고 schema CHECK 를 놓친 결함. 처방: schema.sql 3곳 `('live','testnet','paper')` + 운영 Supabase 에 `ALTER TABLE {t} DROP/ADD CONSTRAINT {t}_mode_check` 즉시 적용(asyncpg, 봇 재시작 불필요 — 다음 기록부터 복구). SCHEMA_FIX 이벤트 INSERT 로 검증. PID 82952 자동 복구 |
| 2026-05-25 | [paper_exchange.py](paper_exchange.py) (신규), [config.py](config.py), [main.py](main.py), [executor.py](executor.py), [test.py](test.py) | **mainnet paper 모드 구현** — testnet 결함 3종(가격 -9% 괴리·얇은 호가 미체결·일봉<201)이 모두 외부 환경이라 코드로 불가 → `MODE=paper` 신규. **`paper_exchange.PaperExchange`**: ccxt.binance 인터페이스를 덕 타이핑으로 흉내. 읽기(fetch_ticker/ohlcv/tickers/load_markets/amount_to_precision/markets)는 mainnet `reader`(ccxt.binance, 키 없음) 위임, 주문/잔고(create_order/cancel_order/fetch_order/fetch_open_orders/fetch_balance)는 로컬 가상 체결. 체결 MVP: limit=현재가 터치 시 즉시 전량 체결(buy 현재가≤지정가/sell 현재가≥지정가), market=현재가±`PAPER_SLIPPAGE`(0.05%), 수수료=`FEE_RATE` USDT 양방향 차감(코인 수량 명목 유지로 그리드 매도 일관성). 미체결 잠금(used) 계산: buy→USDT, sell→coin. 가상 상태(balance·open_orders·next_id)는 `paper_state.json` 원자적 영속화(`os.replace`) → `load_state()` 재시작 복원, 없으면 `SEED/KRW_RATE` USDT 초기 시드. **config**: MODE assert 에 paper 추가 + paper 키 분기(키 None 허용, public 데이터) + `PAPER_STATE_PATH`/`PAPER_SLIPPAGE` 상수. **main**: `MODE=="paper"` 시 `reader=ccxt.binance(...)` → `PaperExchange(reader)` → `load_state()` 주입(set_sandbox 우회). **executor**: `recover_state()` 진입부에 paper 가드 — 가상 상태는 JSON 복원이라 clean-slate 청산을 하면 복원분이 전량 시장가 정리되어 영속화 무의미 → paper 는 RECOVER_STATE INFO 후 즉시 return. **executor·screener·persistence·shared_state·deploy 무변경**(거래소 객체 교체만). 회귀 테스트 5건(limit 체결·market 슬리피지·cancel·state roundtrip·GridEngine 통합) + `test_mode_branch_invalid` 를 demo 로 갱신(paper 가 이제 유효). 전체 단위 테스트 통과. 스모크 테스트(실 mainnet BTC $77,558): 현재가 지정가 즉시 체결·미체결 잠금·JSON 저장 확인. 플랜: `~/.claude/plans/1-sprightly-thunder.md`. **배포 대기**: `git push` → `bash deploy/update.sh` → `.env MODE=paper` → restart |
| 2026-05-25 | [executor.py](executor.py), [shared_state.py](shared_state.py), [test.py](test.py) | **N30 진입 실패 idle·200MA 필터 미적용 가시화 패치** — 도울님 "페이퍼 잘 되나" 질문 → Supabase 직접 진단(asyncpg, statement_cache_size=0)으로 trades 마지막 05-15 22:52·equity_snapshots 는 05-25 현재까지 정상·bot_events 05-15 02:06 이후 무사건 확인 → **10일 무거래 silent idle**. py-spy dump(PID 61100, uptime 10일 17시간): MainThread 가 epoll_wait `ready=[]` 로 정상 idle = **hang 아님**(event loop·screener·heartbeat·snapshot 정상). 서버 testnet 직접 조회: BTC/USDT 일봉 **20개**(<201), 잔고 BTC 없음·열린 주문 0건. 근본 원인 확정: ── 8단계 `setup_grid` 초기 지정가 매수가 testnet 얇은 호가에서 미체결 → [executor.py:350](executor.py#L350) `fill_qty<=0` → return(is_active=False) → engine=None → 60초 후 무한 재시도. 이 idle 은 `_check_n25_trade_idle`([executor.py:120](executor.py#L120) `engine is None` return)·snapshot watchdog(snapshot 정상) 어느 것도 못 잡는 사각지대. **패치(거래 로직 불변, 가시성만)**: ① `BotState.executor_grid_idle_since`/`n30_last_grid_idle_alert_at` + `_note_grid_setup_idle()` 신규 — ── 8 진입 실패가 `N30_GRID_IDLE_ALERT_SEC`(1h) 이상 지속 시 GRID_SETUP_IDLE WARNING(쿨다운 6h, N29 stdout 경로로 journal+텔레그램+Supabase), 진입 성공 시 idle 리셋. ② `BotState.market_filter_unavailable_warned` + `update_market_filter` 일봉<201 시 MARKET_FILTER_UNAVAILABLE WARNING 1회(is_market_healthy 미변형, 매매 허용 유지). 텔레그램 NetworkError 56건은 PTB network_retry_loop 자동 재시도(정상)라 미변경. 회귀 테스트 2건(`test_n30_grid_setup_idle_alerts_after_threshold` — 임계 전 침묵·임계 후 WARNING·쿨다운 spam 방지·리셋 / `test_n30_market_filter_unavailable_warns_once` — 1회 경고·동작 유지). 전체 단위 테스트 통과. 누적 testnet 실현 PnL -233,846원. **배포 대기**: `git push` → `bash deploy/update.sh` → `sudo systemctl restart daily30k`(.service 미변경) |
