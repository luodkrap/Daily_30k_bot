...
**🔴 5/8~5/25 사건 신규 결함 (2026-05-08 추가, 5/25 갱신)**

- [x] 🤖 ~~**PAPER 모드** (2026-05-25 완료): **mainnet 실시세 + 로컬 가상 체결 (`MODE=paper`)** — testnet 결함 3종(가격 -9% 괴리·얇은 호가 setup_grid 미체결·일봉 20개<201 200MA 무력화)이 모두 외부 환경이라 코드로 불가 → testnet 페이퍼 검증 자체가 무의미(누적 -233,846원 신뢰 불가). 처방: mainnet public 데이터를 읽되 주문만 로컬 가상 체결하는 `paper_exchange.PaperExchange`(ccxt.binance 덕 타이핑). 읽기는 mainnet reader 위임, 주문/잔고는 로컬 시뮬레이션, `paper_state.json` 영속화(재시작 복원). 체결 MVP(지정가 터치=전량, 시장가=현재가±0.05% 슬리피지, FEE_RATE). config(MODE assert+paper 키 분기 키 None 허용+PAPER_STATE_PATH/SLIPPAGE), main(exchange 주입 분기), executor(recover_state paper 가드). **executor·screener·persistence·shared_state 무변경**(거래소 객체 교체만). 회귀 테스트 5건 + 스모크(실 mainnet $77,558 즉시 체결 확인). 전체 통과. 다음: `.env MODE=paper` 배포 → 며칠~2주 누적 → B3 재판단(paper 부터 새 카운터). 플랜 `~/.claude/plans/1-sprightly-thunder.md`. **배포 대기**~~
- [x] 🤖 ~~**N30** (2026-05-25 완료): **진입 실패 반복 idle(setup_grid 초기 매수 미체결) + 200MA 필터 미적용 가시화** — 도울님 "페이퍼 잘 되나" 질문 → Supabase 직접 진단으로 trades 마지막 05-15 22:52·bot_events 05-15 02:06 이후 무사건·equity_snapshots 는 05-25 현재까지 정상 → **10일 무거래 silent idle** 발견. py-spy dump(PID 61100, uptime 10일 17시간): MainThread epoll_wait `ready=[]` 정상 idle = **hang 아님**(event loop·screener·heartbeat·snapshot 정상). 서버 testnet 직접 조회: BTC/USDT 일봉 20개(<201)·잔고 BTC 없음·열린 주문 0건. 근본 원인 확정: executor 루프 ── 8단계 `setup_grid` 초기 지정가 매수가 testnet 얇은 호가에서 미체결 → [executor.py:350](executor.py#L350) `fill_qty<=0` → return(is_active=False) → engine=None → 60초 후 무한 재시도. 이 idle 은 `_check_n25_trade_idle`([executor.py:120](executor.py#L120) `engine is None` return)·N25 snapshot watchdog(snapshot 정상) 어느 것도 못 잡는 사각지대였음 → 10일간 N25_TRADE_IDLE 0건. 부수 발견: 일봉 20개<201 로 `update_market_filter` 가 매번 즉시 return → is_market_healthy True 동결·MARKET_FILTER 이벤트 0건. 텔레그램 NetworkError 56건은 PTB network_retry_loop 자동 재시도(정상)라 미변경. **패치(거래 로직 불변, 가시성만)**: ① `BotState.executor_grid_idle_since`/`n30_last_grid_idle_alert_at` + `_note_grid_setup_idle()` 신규 — ── 8 진입 실패 1시간 이상 지속 시 GRID_SETUP_IDLE WARNING(쿨다운 6h, N29 stdout 경로로 journal+텔레그램+Supabase), 진입 성공 시 idle 리셋. ② `BotState.market_filter_unavailable_warned` + `update_market_filter` 일봉<201 시 MARKET_FILTER_UNAVAILABLE WARNING 1회(매매 허용 유지). 회귀 테스트 2건(`test_n30_grid_setup_idle_alerts_after_threshold`, `test_n30_market_filter_unavailable_warns_once`). 전체 단위 테스트 통과. 누적 testnet 실현 PnL -233,846원 → **B3 = live 전환 NO 확정. testnet 결함(가격 피드·호가·일봉) 모두 외부 환경 → mainnet paper 전환 검토가 다음 핵심 의사결정.** **배포 대기**: `git push` → `bash deploy/update.sh` → `sudo systemctl restart daily30k`(.service 미변경)~~
- [x] 🤖 ~~**N29-A/B** (2026-05-15 완료): **KILL_SWITCH 후 봇 사망 침묵 결함 + journal 가시성 부재** — 5/13 13:09 KST testnet 가짜 폭락(mainnet $81,168 vs testnet $73,706, -9% 가격 피드 괴리)으로 손절매 트리거 → 단일 trade -166,021원 → daily_pnl -159,167원 vs limit 90,000원 → KILL_SWITCH 발동 → 봇 정상 종료(exit 0). systemd `Restart=on-failure` 정책상 정상 종료는 재시작 안 함 → **34시간 침묵** (도울님 5/14 23:36 /status 무응답으로 발견). Supabase 직접 진단(`/tmp/diagnose_kill_switch.py`)으로 mode=testnet 확정·실손실 0원 확인. **N29-B 가시성 결함**: `executor._log_event()` 가 텔레그램+Supabase 만 호출하고 stdout 미출력 → KILL_SWITCH 직후 journal 에 종료 라인만 4건, 사건 메시지(KILL_SWITCH/손절매/청산/emergency_sell) 0건. 패치: `_log_event()` 가 severity ∈ {CRITICAL, ERROR, WARN, WARNING} 시 `print(f"[{severity}] {event_type}: {message}{ctx_str}", flush=True)` 동시 호출 (INFO 제외로 폭주 방지). **N29-A 사망 알림 결함**: 봇 프로세스 자체가 죽으면 텔레그램 핸들러도 같이 죽어 봇 내부 알림 불가능 → 사용자가 죽음을 모름. 패치: `deploy/notify_death.sh` 신규 (`.env` 에서 TELEGRAM_TOKEN/CHAT_ID 추출 후 curl 텔레그램 sendMessage, 실패도 exit 0 으로 systemd 재시작 정책 보호) + `daily30k.service` 에 `ExecStopPost=/home/ubuntu/Daily_30k_bot/deploy/notify_death.sh` 추가 — systemd 가 직접 호출하므로 정상 종료·실패·OOM 모두 캐치. `Restart=on-failure` 정책 유지(손실 한도는 명시적 종료가 정답 원칙). 회귀 테스트 2건(`test_n29b_log_event_critical_emits_stdout` — CRITICAL/WARN 출력 + INFO 침묵 + context payload 포함 검증 / `test_n29a_notify_death_script_valid` — 파일 존재·실행권한·TELEGRAM_TOKEN/CHAT_ID/api.telegram.org/exit 0 패턴 + `ExecStopPost` 와 `notify_death.sh` 연결 정적 검증). 전체 단위 테스트 통과. **배포 절차**: `git push` → SSH → `bash deploy/update.sh` → `sudo systemctl daemon-reload` (.service 수정 반영 필수) → `sudo systemctl restart daily30k`. 외부 환경 이슈(testnet 가격 피드 -9% 괴리)는 별개로 mainnet paper 모드 전환 검토 필요~~
- [x] 🤖 ~~**N28** (2026-05-11 완료): **Supabase silent fallback 패치 (asyncpg + pgbouncer 호환)** — N27 commit `fafdf5b` 배포(5/11 17:51 KST) 직후 텔레그램 `[DEGRADED]` 도착. 사유: `DuplicatePreparedStatementError: prepared statement "__asyncpg_stmt_1__" already exists` (asyncpg 가 server-side prepared statement 캐시 사용 + Supabase Transaction Pooler(pgbouncer 6543) 가 transaction-mode 라 prepared statement 가 다른 백엔드에 라우팅되며 이름 충돌). 5/9 배포는 우연히 통과했지만 운영 데이터가 SQLite 로 빠지는 silent fallback 영구 위험. 패치: [persistence.py:192-194](persistence.py#L192-L194) `asyncpg.create_pool(...)` 에 `statement_cache_size=0` 추가 — server-side prepare 우회, asyncpg 가 매 쿼리 inline parameter 송신. 봇 쿼리 빈도(초당 1회 미만)라 성능 영향 무시. 회귀 테스트 1건(`test_n28_supabase_pool_disables_statement_cache`): 정적 검증으로 코드 패턴 존재 확인 (실제 connection 통합 테스트는 CI 의존성으로 회피). 전체 단위 테스트 통과. **배포 대기**: `bash deploy/update.sh`~~
...
- [x] 🤖 ~~**N25** (2026-05-13 완료): **engine idle 재발 감시 체계 보강** — 5/5 16:48 KST 73시간 정지 + 5/9 00:02:37 KST 14시간+ 정지 재발의 공통 신호가 `equity_snapshot` 30분 타이머 침묵이었음. 패치: `BotState` 에 `executor_last_snapshot_at` / `executor_last_trade_at` / `n25_last_trade_idle_alert_at` 추가. `snapshot_equity()` 성공 시 snapshot progress 갱신, `_log_trade(..., state=...)` 성공 시 trade progress 갱신. `main._supervise()` 에 heartbeat 와 별도 `progress_timeout` / `progress_attr` watchdog 추가 — executor heartbeat 가 살아있어도 `executor_last_snapshot_at` 이 45분 이상 무갱신이면 task cancel → `TimeoutError` → 기존 `SUPERVISOR_RESTART` 경로로 재시작. `run_executor()` 는 부팅 시 progress 시각 초기화 + 활성 엔진 상태에서 24시간 무거래 시 `N25_TRADE_IDLE` WARNING 이벤트/텔레그램 경고(alert-only, 정상 저변동장 청산 방지). 회귀 테스트 2건(`test_n25_supervise_restarts_when_snapshot_stale`, `test_n25_snapshot_and_trade_mark_progress`) 추가. 남은 운영 규칙: 배포 후 Supabase `SUPERVISOR_RESTART is_watchdog=true` / `N25_TRADE_IDLE` 발생 여부 관찰, 실제 재발 시 여전히 `py-spy dump --pid <PID>` 로 원인 await 확보~~
- [x] 🤖 ~~**B3-ADV-01 F1** (2026-05-13 완료): **긴급 청산 잠긴 잔고 결함 (CRITICAL)** — Codex 적대적 리뷰가 needs-attention 평가로 적발. `check_stop_loss`/`emergency_sell` 가 시장가 매도 선행 → `cancel_all` 후행 순서라 Binance spot 의 열린 limit sell 이 보유 코인을 used 잔고로 잠가 free 부족 시 시장가 전량 매도 거절될 수 있음. 손절·일일손실·수익중단·코인스위칭·봇종료 5곳 모두 동일 경로. 패치: 새 헬퍼 `_safe_liquidate(reason) -> bool` 로 5곳 통합 → `cancel_all` 선행 → 0.5초 대기 → `fetch_balance` free 재동기화 → `amount_to_precision` 보정 후 시장가 매도 → 실패 시 상태(`total_qty`/`avg_price`) 유지 + `LIQUIDATE_FAILED` CRITICAL + 텔레그램 알림 + 다음 루프 재시도. `regrid()` 동일 결함도 같이 정리. 회귀 테스트 2건(`test_codex_f1_emergency_sell_cancels_first` — 호출 순서 로그 검증 / `test_codex_f1_emergency_sell_uses_free_balance` — `MockExchangeWithLockedBalance` 잠긴 잔고 + InsufficientBalance 예외 시뮬레이션). 전체 단위 테스트 통과~~
- [x] 🤖 ~~**B3-ADV-01 F2** (2026-05-13 완료): **monitor_orders 외부 취소 오인 결함 (HIGH)** — Codex 적대적 리뷰 적발. `fetch_open_orders` 결과에 ID 가 없으면 `fetch_order` 없이 곧바로 `_handle_*_fill` 호출 → 외부 취소·expired·rejected·부분 체결 후 취소가 모두 "완전 체결" 로 처리되어 `total_qty`/`avg_price`/PnL 오염. 패치: 새 헬퍼 `_resolve_missing_order(side)` 에서 `fetch_order` 로 status/filled/average 재확인 후 분기 — closed+filled>0 정상 체결, canceled+filled>0 부분 반영 + `ORDER_PARTIAL_CANCEL` WARNING, canceled+filled=0 무변경 + `ORDER_CANCELED` INFO, pending/open 보류. `_handle_buy_fill`/`_handle_sell_fill` 시그니처에 `actual_avg_price`·`actual_fill_qty` 옵셔널 인자 추가(기존 호출자 호환). 회귀 테스트 2건(`test_codex_f2_open_order_missing_fetches_status` / `test_codex_f2_partial_fill_uses_actual_filled` — `MockExchangePartialFill` 로 filled=0.3 응답 주입). 전체 단위 테스트 통과~~
- [x] 🤖 ~~**B3-ADV-01 F3** (2026-05-13 완료): **\_limit_buy_with_retry 부분 체결 유실 결함 (HIGH)** — Codex 적대적 리뷰 적발. 타임아웃 시 `cancel_order` 만 호출하고 `filled` 미확인 → 부분 체결분(예: 1.0 BTC 요청 → 0.6 BTC 체결 후 취소) 이 `(0.0, 0.0)` 반환되어 그리드 미배치 또는 재시도로 의도 초과 매수. 패치: cancel 직후 `fetch_order` 재조회 추가, `filled > 0` 이면 `(avg, filled)` 반환·재시도 중단으로 보유분 회수. 회귀 테스트 2건(`test_codex_f3_limit_buy_returns_partial_fill` — filled=0.6 회수 검증 / `test_codex_f3_limit_buy_full_zero_retries` — filled=0 케이스 `(0.0, 0.0)` 회귀 방지). 전체 단위 테스트 통과~~
- [x] 🤖 ~~**B3-ADV-01 F4** (2026-05-13 완료): **텔레그램 명령 권한 검증 부재 (HIGH)** — Codex 적대적 리뷰 적발. `/status`·`/stop`·`/seed` 핸들러에 `update.effective_chat.id` 검증 없음 → 봇 사용자명·토큰 노출 시 제3자가 즉시 봇 중단(`/stop`) 또는 SEED 변경(`/seed`)으로 실거래 포지션 사이징 조작 가능. 패치: `config.py` `TELEGRAM_CHAT_ID_INT` (int 변환, None 가드 포함) 추가 + `main.py` 3개 CommandHandler 에 `filters.Chat(chat_id=TELEGRAM_CHAT_ID_INT)` 적용. `TELEGRAM_CHAT_ID_INT=None` 시 봇이 부팅 텔레그램에 CRITICAL 알림 후 `kill_event.wait()` 으로 안전 멈춤. 부팅 메시지에 `[Telegram] 허가 chat_id=<int>` 한 줄 추가해 운영 확인. notifier outbound 는 `TELEGRAM_CHAT_ID` (str) 유지 — Telegram API 호환. 회귀 테스트 1건(`test_codex_f4_unauthorized_chat_rejected` — `filters.Chat.chat_ids` 검증 + `TELEGRAM_CHAT_ID_INT` 변환 로직 검증). 전체 단위 테스트 통과~~
...
### 🤖 장기 보완 백로그 (2026-05-14 사용자 요청)

> Codex 설계 리뷰 대화에서 도출. 당장 구현하지 않더라도 live 전환 전후로 하나씩 집어 올릴 개선 후보. 핵심 방향은 "전략 기대값 증명 + 거래소 실제 상태와 봇 내부 상태 불일치 제거 + live 전환 안전장치 자동화".

- [ ] 🤖 **S1 (High): DB 기반 포지션·주문 상태 복구 모드** — 현재 재시작 복구는 안전 우선의 전량정리 방식(`recover_state`: 미체결 주문 취소 + 비-USDT 잔고 시장가 매도). 다음 단계에서는 `trades`/`bot_events`/거래소 `fetch_open_orders`/잔고를 대조해 마지막 포지션, 평균단가, 열린 주문을 이어받는 "resume" 모드와 기존 "liquidate-and-reset" 모드를 선택 가능하게 한다. 단, live 기본값은 충분히 검증될 때까지 기존 전량정리 유지.
- [ ] 🤖 **S2 (High): 거래소 reconciliation 루프** — 주기적으로 봇 내부 상태(`GridEngine.total_qty`, `avg_price`, `buy_orders`, `sell_orders`)와 Binance 실제 잔고/미체결 주문/최근 체결을 대조. 불일치 발견 시 `RECONCILE_MISMATCH` 이벤트 + 텔레그램 경고, 안전한 경우 내부 상태 보정, 위험한 경우 신규 주문 중단. F2/F3 이후에도 실거래 지연·외부 취소·부분 체결 경계값 방어용.
- [ ] 🤖 **S3 (Medium-High): live 전환 preflight 체크리스트 자동화** — `MODE=live` 부팅 전 또는 `deploy/advise.sh` 에서 출금 권한 OFF, IP 화이트리스트, `BINANCE_API_KEY`/`BINANCE_TESTNET_API_KEY` 혼선, `TELEGRAM_CHAT_ID_INT`, Supabase backend, 최근 테스트 통과 여부를 점검하는 `python -m reports.preflight_live` 또는 배포 가드 추가. 실패 시 live 전환 절차 중단.
- [ ] 🤖 **R4 (Medium): 전략 기대값·리스크 리포트 강화** — R3 대시보드에 월간 기대값, 거래당 평균 순손익(수수료 차감), 연속 손실일, MDD, 일별 손익 분포, 수수료 비중, 코인 스위칭 비용을 포함. "하루 3만 원 달성 여부"보다 live 판단에 중요한 장기 기대값과 손실 꼬리 확인 목적.
- [ ] 🤖 **M1 (Medium): 침묵 자체를 경고하는 운영 알림 확장** — N25 는 executor snapshot 침묵에 집중. 후속으로 screener 후보 미갱신, Telegram 발송 실패 누적, Supabase fallback 지속, `notify_status` 장기 미발송, 24시간 거래 0건 등을 통합한 `run_health_monitor()` 컴포넌트 검토. 모든 알림은 중복 방지·쿨다운 필수.
...
- [ ] 🤖 백테스트 엔진 구축 (C 트랙, 2026-04-21 플랜 승인, **N 트랙 Top 3 해소 후**). 구현 시 수수료·슬리피지·지정가 미체결·급락장·코인 스위칭 비용을 반드시 시뮬레이션에 포함:
